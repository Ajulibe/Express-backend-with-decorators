...
export class Add {
  ...
  public async profileImage(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  @joiValidation(addImageSchema)
  public async backgroundImage(req: Request, res: Response, next: NextFunction): Promise<void> {
    const { version, publicId }: IBgUploadResponse = await Add.prototype.backgroundUpload(req.body.image, next);
  ...
  private async backgroundUpload(image: string, next: NextFunction): Promise<IBgUploadResponse> {
